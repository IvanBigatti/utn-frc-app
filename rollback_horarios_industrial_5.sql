-- ============================================================
-- Rollback de carga_horarios_industrial_5.sql
--
-- Restaura las 30 filas de ComisionMaterias de 5D1..5D5
-- (120..124) desde el snapshot que tomó el script de carga.
--
...
DO $guard$
BEGIN
  IF to_regclass('backup._bkp_industrial5_comisionmaterias') IS NULL THEN
    RAISE EXCEPTION 'No existe el snapshot backup._bkp_industrial5_comisionmaterias';
  END IF;
END $guard$;
...
UPDATE public."ComisionMaterias" cm
  ...
  FROM backup._bkp_industrial5_comisionmaterias b
...
BEGIN
  SELECT count(*) INTO v
    ...
    JOIN backup._bkp_industrial5_comisionmaterias b
  ...
  RAISE NOTICE 'OK: 5D1..5D5 restauradas al estado previo a la carga.';
END $verif$;

COMMIT;

-- Cuando ya no haga falta:
-- DROP TABLE backup._bkp_industrial5_comisionmaterias;
