-- ============================================================
-- Rollback de carga_horarios_industrial_2.sql
--
-- Restaura las 32 filas de ComisionMaterias de 2D1..2D4
-- (110..113) desde el snapshot que tomó el script de carga.
--
...
DO $guard$
BEGIN
  IF to_regclass('backup._bkp_industrial2_comisionmaterias') IS NULL THEN
    RAISE EXCEPTION 'No existe el snapshot backup._bkp_industrial2_comisionmaterias';
  END IF;
END $guard$;
...
UPDATE public."ComisionMaterias" cm
  ...
  FROM backup._bkp_industrial2_comisionmaterias b
...
BEGIN
  SELECT count(*) INTO v
    ...
    JOIN backup._bkp_industrial2_comisionmaterias b
  ...
  RAISE NOTICE 'OK: 2D1..2D4 restauradas al estado previo a la carga.';
END $verif$;

COMMIT;

-- Cuando ya no haga falta:
-- DROP TABLE backup._bkp_industrial2_comisionmaterias;
